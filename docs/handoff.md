...
- **Phase 1-O (회계 결산 및 총회 감사 보고서 생성기)**:
  ...
  - 한글 깨짐 없는 `\uFEFF` UTF-8 BOM 엑셀(CSV) 감사 원장 데이터 즉시 다운로드 지원
- **Supabase**: `nexus-dev` / `brknzpcbabwobihxjseq` / Seoul 프로젝트. 총 12개 마이그레이션 적용 완료. 22개 테이블 RLS 보호 및 22개 PGlite 테스트 100% 통과.
